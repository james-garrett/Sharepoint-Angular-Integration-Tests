            ...
            <div id="contract" class="row">
                ...
                <ul style="list-style: none;"> 
                    <div class="row" ng-repeat="item in list track by item.Id">
                        <div class="col-sm-9">
                            <iframe src="{{getFile(item)}}" width='{{screenWidth}}' height='{{screenHeight}}' frameborder='0'>This is an embedded <a target='_blank' href='http://office.com'>Microsoft Office</a> document, powered by <a target='_blank' href='http://office.com/webapps'>Office Online</a>.</iframe>
                        </div>
                        <!-- <li ng-if="!itemIsSubsection(item.SubSectionNo)" class="col-sm-9" id="contractBody"> -->
                        <!-- <div class="card-body bg-light mb-3 card">
                            <h1 class="card-title">{{item.SectionNo}}{{item.SubSectionNo}}    <b>{{item.Title}}</b></h1>
                            <p class="card-text">{{item.SectionText}}</p>
                        </div> -->
                    ...
                    <!-- <li ng-if="itemIsSubsection(item.SubSectionNo)" class="col-sm-9" id="contractBody"> -->
                        <!-- <div class="card-body bg-light mb-3 card"> -->
                            ...
                            <!-- < class="card-text"> -->
                                <!-- {{item.SectionText}} -->
                            <!-- </p> -->
                            <!-- https://jamesg.sharepoint.com/sites/test1/Contract%20Library/Preamble.docx?d=w114e4b13efd04f5ab548ae2ace419f85 -->
                            <!-- <iframe src="{{getFile(item[$index])}}" width='1366px' height='623px' frameborder='0'>This is an embedded <a target='_blank' href='http://office.com'>Microsoft Office</a> document, powered by <a target='_blank' href='http://office.com/webapps'>Office Online</a>.</iframe> -->
                    <!-- 'https://view.officeapps.live.com/op/embed.aspx?src=http://remote.url.tld/path/to/document.doc'  -->
                        <!-- </div> -->
                    <!-- </li> -->
                    <div class="col-sm-3 card bg-light mb-3" id="userReport" ng-if="getAssociatedUsers($index, 0)">
                        <div ng-repeat="user in getAssociatedUsers($parent.$index, $index) track by user.Id" id="userPanel">
                            <p>{{user.title}} <bold>&#10004</bold></p>
                        </div> 
                    </div>
                    </div>
                        <form>
...
function contractRendererController($scope) {
    ...
    // mainApp.controller("listHandlerController", function($scope, $q) {  
        ...
        $scope.screenWidth = "700px";
        $scope.screenHeight = "400px";
        ...
        var sortListResults = function(list) {
            // list.sort(dynamicSortMultiple(["Section_x0020_Number", "SubSectionNo"]));
            list.sort(dynamicSort("Section_x0020_Number", 1));
            // console.log(list);
            return list;
        }
        ...
        var setTextData = function(data) {
            ...
            $scope.$apply(function() {
                ...
                $scope.list = sortListResults(data);
            });
            return $scope.list;
        }
        ...
        var readList = function(complete, failure, rootURL, endpoint) {
            ...
            // var rootURL = "https://jamesg.sharepoint.com/sites/test1/Contract%20Library/Forms/AllItems.aspx"
            ...
            // var endpoint = "/_api/Web/Lists/getbytitle('Contract Library')/Files";
            // var endpoint = "/_api/Web/Lists/getbytitle('Contract Library')/items(32)?$select=*,Folder/UniqueId&$expand=Folder";
            var endpoint = "/_api/Web/Lists/getbytitle('Contract Library')/items";
        ...
        $scope.getFile = function(item) {
            ...
            // console.log(item["__metadata"]["uri"]);
            // return item["__metadata"]["id"];
            console.log(item["ServerRedirectedEmbedUrl"].replace('https', 'http'));
            // return item["ServerRedirectedEmbedUrl"].replace('https', 'http');
            return item["ServerRedirectedEmbedUrl"];
            // return item["Url"] + '?d=w698bb3615ea642e2a8d57c17a4e7eb4e';
        };
        ...
        /***
         * var getListActions = elmt => {
         *      List of events to track as observables
         * 
         *      elmt.listItemsAdded
         *      elmt.listItemsRemoved
         *      elmt.listItemsRead
         *      elmt.listItemFocused
         *      elmt.listItemsApproved
         *      elmt.picturesAdded
         *      elmt.notesAdded
         *      elmt.userJoined
         *      elmt.userLeft
         *      return elmt(event => document.event)
         * }
         * 
         * getElementDrags(image).forEach(status => image.status => status);
         */

        //  const listItems = new Observable((observer) => {
